...
export default {
  ...
  config: {
    ...
    autoGenerateOnSave: {
      ...
      default: true,
      description:
        "When you are in a file and save - it will automatically trigger generation of SVGs"
    }
  },
  activate(state) {
    this.managers.push({
    ...
    this.managers.push({
      name: "autocompleter",
      obj: new WikiCompleter(this, state)
    });
  },
  ...
  getAutoCompleteProviders() {
    ...
    if (this.managers) {
      for (var manager of this.managers) {
        if (manager.obj && manager.obj.getAutoCompleteProviders) {
          providers = providers.concat(manager.obj.getAutoCompleteProviders());
        }
      }
    }
    return providers;
