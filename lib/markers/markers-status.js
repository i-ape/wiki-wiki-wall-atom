...
export default class MarkersStatus {
  ...
  getText() {
    if (
      ...
      this.editor.buffer.file.path.match(/\.wwwf$/)
    ) {
      return "WWWF";
  ...
  observeEvents() {
    ...
    this.subscriptions.add(
      this.workspace.observeActiveTextEditor(editor => {
  ...
  updateTooltip() {
    ...
    if (this.editor) {
      const tooltipText = `Active editor is on a wwwf file ${this.editor.buffer
        .file.path}`;
